...
def sample_strategy(stragegy: Literal['long', 'short', 'swing'] = 'swing', signal_only=True, slow=90, fast=20) -> Dict[Asset, pd.Series | pd.DataFrame]:
    result = {}
    for a, f in FRAMES.items():
        df = f[["Close"]]
        df["ma20"] = df["Close"].rolling(fast).mean()
        df["ma90"] = df["Close"].rolling(slow).mean()
        ...
        if stragegy == 'swing':
            ...
            # FIXME short cant use percent order !!
